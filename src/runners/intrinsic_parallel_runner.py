...
class ParallelRunner:
    ...
    def setup(self, scheme, groups, preprocess, mac, exp_mac, intrinsic_mac):
    ...
    def get_unit_types(self):
        ...
        for batch_idx in range(self.batch_size):
            obj_refs.append(self.env_workers[batch_idx].work.remote("get_unit_types", None))
    ...
    def run(self, test_mode=False):
        try:
            ...
            # if test_mode:
            #     self.intrinsic_mac.init_hidden(batch_size=self.batch_size)
            ...
            while True:
                ...
                _, _, _, z_for_policy_outs = self.exp_mac.encoder_forward(self.batch, self.t)
                # if test_mode:
                #     actions = self.intrinsic_mac.intrinsic_select_actions(z_for_policy_outs, self.batch, t_ep=self.t, t_env=self.t_env, bs=envs_not_terminated, test_mode=test_mode)
                # else:
                actions = self.mac.intrinsic_select_actions(z_for_policy_outs, self.batch, t_ep=self.t, t_env=self.t_env, bs=envs_not_terminated, test_mode=test_mode)
    ...
    def work(self, cmd, data):
        try:
            if cmd == "step":
                ...
                return self.env.get_unit_types()
            else:
