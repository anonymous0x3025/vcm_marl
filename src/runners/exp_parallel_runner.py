...
class ExpParallelRunner:
    ...
    def __init__(self, args, logger):
        ...
        self.train_returns = []
        self.test_returns = []
        self.train_mixed_returns = []
        self.test_mixed_returns = []
        self.train_stats = {}
        self.test_stats = {}
    ...
    def setup(self, scheme, groups, preprocess, mac, exp_mac):
    ...
    def run(self, test_mode=False):
        try:
            ...
            mixed_episode_returns = [0 for _ in range(self.batch_size)]
            episode_lengths = [0 for _ in range(self.batch_size)]
            self.mac.init_hidden(batch_size=self.batch_size)
            if self.args.ENCODER_RNN:
                self.exp_mac.init_hidden(batch_size=self.batch_size)
            ...
            while True:
                ...
                for data, idx in data_list:
                    ...
                    mixed_episode_returns[idx] += data["reward"]
                ...
                int_rewards = self.exp_mac.calc_int_reward(self.batch, self.t, self.t_env).to("cpu").detach().numpy()

                envs_not_terminated_idx = 0
                for batch_idx in range(self.batch_size):
                    if not _terminated[batch_idx]:
                        post_transition_data["reward"][envs_not_terminated_idx][0] += int_rewards[batch_idx]
                        mixed_episode_returns[envs_not_terminated_idx] += int_rewards[batch_idx]
                        envs_not_terminated_idx += 1

                # Add post_transiton data into the batch
                post_transition_data = convert_format_to_numpy_array(post_transition_data)
                self.batch.update(post_transition_data, bs=envs_not_terminated, ts=self.t - 1, mark_filled=False)
            ...
            cur_mixed_returns = self.test_mixed_returns if test_mode else self.train_mixed_returns
            log_prefix = "test_" if test_mode else ""
            log_prefix_for_mixed = "test_mixed_" if test_mode else "mixed_"
            ...
            cur_mixed_returns.extend(mixed_episode_returns)

            n_test_runs = max(1, self.args.TEST_NEPISODE // self.batch_size) * self.batch_size
            if test_mode and (len(self.test_returns) == n_test_runs):
                self.test_return_for_model_saving = np.mean(cur_returns)
                self._log(cur_returns, cur_stats, log_prefix)
                self._log(cur_mixed_returns, cur_stats, log_prefix_for_mixed)
