...
class ExpMAC:
    ...
    def calc_mse_loss(self, inputs_decoded, target_y, state_decoded, state_y):
        exp_prediction_diff = inputs_decoded - target_y
        exp_state_decoding_diff = state_decoded - state_y
        exp_prediction_error = (exp_prediction_diff ** 2).mean(2).mean(1)
        exp_state_decoding_error = (exp_state_decoding_diff ** 2).mean(1)
        # exp_prediction_error = self.mse_loss(inputs_decoded, target_y).mean(2).mean(1)
        # exp_state_decoding_error = self.mse_loss(state_decoded, state_y).mean(1)
    ...
    def _build_agents(self):
        ...
        self.exp_ae.share_memory()
        self.target_exp_ae.share_memory()
        self.exp_sd.share_memory()
        self.target_exp_sd.share_memory()
