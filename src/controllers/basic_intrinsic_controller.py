...
class BasicIntrinsicMAC:
    ...
    def intrinsic_select_actions(self, z_outs, ep_batch, t_ep, t_env, bs=slice(None), test_mode=False):
        # Only select actions for the selected batch elements in bs
        agent_outputs = self.intrinsic_forward(z_outs, ep_batch, t_ep, test_mode=test_mode)
        avail_actions = ep_batch["avail_actions"][:, t_ep]
        chosen_actions = self.action_selector.select_action(agent_outputs[bs], avail_actions[bs], t_env, test_mode=test_mode)
        return chosen_actions

    def util_select_actions(self, util_outputs, z_outs, ep_batch, t_ep, t_env, bs=slice(None), test_mode=False):
        # Only select actions for the selected batch elements in bs
        agent_outputs = self.util_forward(z_outs, ep_batch, t_ep, test_mode=test_mode)
        avail_actions = ep_batch["avail_actions"][:, t_ep]
        chosen_actions = self.action_selector.select_action(util_outputs[bs], agent_outputs[bs], avail_actions[bs], t_env, test_mode=test_mode)
        return chosen_actions
    ...
    def intrinsic_forward(self, z_outs, ep_batch, t, test_mode=False):
        # if self.epsilon != 1.0:
            # print(self.epsilon)
        agent_inputs = self._build_inputs(ep_batch, t)
    ...
    def util_forward(self, z_outs, ep_batch, t, test_mode=False):
        # if self.epsilon != 1.0:
            # print(self.epsilon)
        agent_inputs = self._build_inputs(ep_batch, t)
        agent_inputs = th.cat([agent_inputs, z_outs], dim=1)
        agent_outs, self.hidden_states = self.agent(agent_inputs, self.hidden_states)
        return agent_outs.view(ep_batch.batch_size, self.n_agents, -1)
