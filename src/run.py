...
def run(_config, run_number=0):
    ...
    # Save stats
    hidden_policy = '-no_hidden' if (not args.HIDDEN_POLICY and args.AE_EXPLORATION) else ''
    use_attention = '-no_attention' if (not args.USE_ATTENTION) else ''
    use_int_reward = '-no_int_reward' if (not args.USE_INT_REWARD) else ''
    if "ices" in args.ALGORITHM:
        save_path = os.path.join(args.LOCAL_RESULTS_PATH, "stats", args.ALGORITHM + hidden_policy + use_attention + use_int_reward, args.ENV)
    else:
        beta = '-Beta_' + str(args.BETA)
        rho = '-Rho_' + str(args.RHO)
        save_path = os.path.join(args.LOCAL_RESULTS_PATH, "stats",
                                 args.ALGORITHM + hidden_policy + use_attention + use_int_reward + beta + rho, args.ENV)
    make_dirs(save_path)
...
def run_sequential(args, logger):
    ...
    use_int_reward = '-no_int_reward' if (not args.USE_INT_REWARD) else ''
    if "ices" in args.ALGORITHM:
        model_save_path = os.path.join(args.LOCAL_RESULTS_PATH, "models",
                                       args.ALGORITHM + hidden_policy + use_attention + use_int_reward,
                                       args.ENV, args.MAP)
    else:
        beta = '-Beta_' + str(args.BETA)
        rho = '-Rho_' + str(args.RHO)
        model_save_path = os.path.join(args.LOCAL_RESULTS_PATH, "models", args.ALGORITHM + hidden_policy + use_attention + use_int_reward + beta + rho, args.ENV, args.MAP)
    ...
    # Learner
    learner_class = le_REGISTRY[args.LEARNER]
    if args.AE_EXPLORATION:
        ...
        if args.HIDDEN_POLICY:
            ...
            learner = learner_class(mac, exp_mac, buffer.scheme, logger, args, hidden_mac)
        else:
            if args.RUNNER.find('intrinsic') >= 0:
                if args.USE_INTRINSIC_MAC:
                    intrinsic_mac = mac_REGISTRY[args.INTRINSIC_MAC](buffer.scheme, groups, args)
                    learner = learner_class(mac, exp_mac, buffer.scheme, logger, args, intrinsic_mac=intrinsic_mac)
                else:
                    learner = learner_class(mac, exp_mac, buffer.scheme, logger, args)
            else:
                learner = learner_class(mac, exp_mac, buffer.scheme, logger, args)
    else:
        if args.LEARNER == "ices_nq_learner":
            mac = mac_REGISTRY[args.MAC](buffer.scheme, groups, args)
            learner = learner_class(mac, buffer.scheme, logger, args, env_info)
        else:
            mac = mac_REGISTRY[args.MAC](buffer.scheme, groups, args)
            learner = learner_class(mac, buffer.scheme, logger, args)

    if args.RUNNER.find('exp') >= 0 or args.RUNNER.find('util') >= 0 or args.RUNNER.find('surprise') >= 0:
        runner.setup(scheme=scheme, groups=groups, preprocess=preprocess, mac=mac, exp_mac=exp_mac)
    elif args.RUNNER.find('intrinsic') >= 0:
        runner.setup(scheme=scheme, groups=groups, preprocess=preprocess, mac=mac, exp_mac=exp_mac, intrinsic_mac=intrinsic_mac)
    else:
        if args.HIDDEN_POLICY:
    ...
    # logger.log_info("Beginning training for {} timesteps".format(args.T_MAX))

    if args.ALGORITHM == "emc":
    ...
    while train_iteration <= args.TRAIN_STEP_MAX:
        ...
        if buffer.can_sample(args.BATCH_SIZE) and episode >= args.START_TRAINING_EPISODE:
            if args.ALGORITHM == 'liir':
                n_train = 1
            else:
                n_train = int(args.BATCH_SIZE_RUN / 2) + 1
            for train_epoch in range(n_train):
                ...
                logger.log_stat("t_env", runner.t_env, runner.t_env)
            ...
            if args.ALGORITHM.find('intrinsic') >= 0 or args.ALGORITHM.find('exp') >= 0:
                learner.update_int_reward_scale(train_iteration)
        ...
        if (runner.t_env - last_test_T) / args.TEST_INTERVAL >= 1.0:
            ...
            # logger.log_info("t_env: {}/{}".format(runner.t_env, args.T_MAX))
            ...
            if args.SAVE_MODEL and current_test_return_for_model_saving >= last_test_return_for_model_saving:
                last_test_return_for_model_saving = copy.deepcopy(current_test_return_for_model_saving)
