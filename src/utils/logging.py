...
class Logger:
    ...
    def set_wandb(self, args):
        ...
        # else:
        #     self.wandb = None
        hidden_policy = '-no_hid' if (not args.HIDDEN_POLICY and args.AE_EXPLORATION) else ''
        use_attention = '-no_att' if (not args.USE_ATTENTION) else ''
        use_int_reward = '-no_ir' if (not args.USE_INT_REWARD) else ''
        use_n_step = '' if not args.N_STEP else '-{}_step'.format(args.N_STEP)
        start_time = time.strftime('%y-%m-%d/%X', time.localtime(time.time()))

        if "ices" in args.ALGORITHM:
            wandb_name = "{}/{}/{}/{}".format(args.ENV,
                                              args.ALGORITHM + hidden_policy + use_attention + use_int_reward + use_n_step,
                                              args.MAP, start_time)
        else:
            beta = '-Beta_' + str(args.BETA)
            rho = '-Rho_' + str(args.RHO)
            wandb_name = "{}/{}/{}/{}".format(args.ENV,
                                              args.ALGORITHM + hidden_policy + use_attention + use_int_reward + use_n_step + beta + rho,
                                              args.MAP, start_time)
