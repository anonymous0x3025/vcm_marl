...
class QLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        for t in range(batch.max_seq_length):
            agent_outs = self.mac.forward(batch, t=t)
            mac_out_for_discrim.append(agent_outs)
        mac_out_for_discrim = th.stack(mac_out_for_discrim, dim=1)  # Concat over time

        # Discriminator
        mac_out_for_discrim[avail_actions == 0] = -9999999
        q_softmax_actions = th.nn.functional.softmax(mac_out_for_discrim[:, :-1], dim=3)

        if self.args.HARD_QS:
            maxs = th.max(mac_out_for_discrim[:, :-1], dim=3, keepdim=True)[1]
    ...
    def to_device(self):
        ...
        self.discrim.to_device()
        if self.args.RNN_DISCRIM:
            self.rnn_agg.to_device()
        if self.mixer is not None:
