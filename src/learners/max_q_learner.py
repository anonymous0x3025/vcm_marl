...
class MAXQLearner:
    ...
    def _train(self, batch: EpisodeBatch, t_env: int, episode_num: int, additional_loss=None, intrinsic_rewards=None, z_out=None):
        ...
        for t in range(batch.max_seq_length):
            if z_out is None:
                ...
                agent_outs = self.central_mac.intrinsic_forward(z_out[:, t].detach(), batch, t=t)
        ...
        for t in range(batch.max_seq_length):
            if z_out is None:
                ...
                target_agent_outs = self.target_central_mac.intrinsic_forward(z_out[:, t].detach(), batch, t=t)
    ...
    def to_device(self):
        ...
        if self.central_mac is not None:
            self.central_mac.to_device()
            self.target_central_mac.to_device()
        self.central_mixer.to(device=self.mac.device)
        self.target_central_mixer.to(device=self.mac.device)
